...
Any MCP client can use this server (this repo's chat client, Claude Code,
Claude Desktop, MCP Inspector). Never print to stdout here — stdio carries the
...
def get_trial_balance(
    ...
    Defaults: configured business unit/ledger, current fiscal year and period.
...
def tb_integrity_check(
    ...
    """Trial-balance health check through a period: does the TB net to zero, suspense
    account balances, accounts missing chartfield definitions, inactive accounts with
    balances, unposted journals, posted journals that don't balance, and whether
    beginning balances roll correctly from the prior-year close (retained earnings)."""
    return _safe(
