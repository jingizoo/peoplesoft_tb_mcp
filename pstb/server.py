...
def _table_health(table: str, source: str) -> dict:
    ...
    The engine itself lives in pstb/health.py with its dependencies
    injected (the ticker runs it too); this wrapper binds the MCP
    module's globals and changes nothing else.
    """
    from .health import table_health

    name, catalog = _metadata_for_source(source)
    return table_health(
        table, source_name=name, catalog=catalog,
